...
def capture_claude_token() -> tuple[bool, str]:
    """Authorize Claude fully automatically via in-app OAuth (localhost
    redirect) — browser opens, user authorizes, we catch the code and exchange
    it for the token. No paste, no terminal. Returns (ok, message)."""
    from . import claude_oauth
    return claude_oauth.authorize()
